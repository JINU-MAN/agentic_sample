...
def _ensure_session_log_dir() -> Path:
    ...
    base_name = f"session_log_{_event_file_timestamp(datetime.now(timezone.utc))}"
    candidate = LOG_DIR / base_name
    suffix = 1
    while candidate.exists():
        candidate = LOG_DIR / f"{base_name}_{suffix}"
        suffix += 1
    candidate.mkdir(parents=True, exist_ok=True)
    _SESSION_LOG_DIR = candidate
    return candidate


def _session_event_file_path(component: str, action: str, event_ts: datetime) -> Path:
    session_log_dir = _ensure_session_log_dir()
    safe_component = _sanitize_component_name(component)
    safe_action = _sanitize_component_name(action)
    ts_token = _event_file_timestamp(event_ts)
    base_name = f"{safe_component}_{safe_action}_{ts_token}"
    candidate = session_log_dir / f"{base_name}.json"

    suffix = 1
    while candidate.exists():
        candidate = session_log_dir / f"{base_name}_{suffix}.json"
        suffix += 1
    return candidate
...
def initialize_process_logging() -> None:
    ...
    - creates one readable session directory under log/.
    """
...
def log_event(
    ...
    *,
    ...
    try:
        ...
        with _LOCK:
            session_event_file = _session_event_file_path(component=component, action=action, event_ts=event_ts)
            _write_line(SYSTEM_LOG_FILE, payload)
            _write_line(component_file, payload)
            _write_json(session_event_file, payload)
    except Exception:
